...
void test_root_of_trust_verify(void)
{

	/* Success. */
	int retval = root_of_trust_verify(pk, pk_hash, sig, firmware,
			sizeof(firmware));

	zassert_equal(0, retval, "retval was %d", retval);

	retval = root_of_trust_verify(const_pk, const_pk_hash, const_sig, const_firmware,
	...
	pk[1]++;
	retval = root_of_trust_verify(pk, pk_hash, sig, firmware,
			sizeof(firmware));
	pk[1]--;
...
void test_main(void)
{
	ztest_test_suite(test_bl_crypto,
			 ztest_unit_test(test_root_of_trust_verify),
