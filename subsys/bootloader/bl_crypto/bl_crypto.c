...
static int _root_of_trust_verify(
...
{
	return _root_of_trust_verify(public_key, public_key_hash, signature,
					firmware, firmware_len, false);
}


/* For use through ext_abi. */
int root_of_trust_verify_external(
			const u8_t *public_key, const u8_t *public_key_hash,
			const u8_t *signature, const u8_t *firmware,
			const u32_t firmware_len)
{
...
__ext_abi(struct rot_verify_abi, rot_verify_abi) = {
	.header = ABI_INFO_INIT(ROT_VERIFY_ABI_ID, ROT_VERIFY_ABI_FLAGS,
				ROT_VERIFY_ABI_VER, sizeof(struct rot_verify_abi)),
	.abi = {
		.root_of_trust_verify = root_of_trust_verify_external,
	}
};
